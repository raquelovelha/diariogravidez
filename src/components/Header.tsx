...
import { Home, MessageCircle, Book, Info } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ currentView, onChangeView }) => {
  return (
    <header className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-50 border-b border-dd-primary/10 w-full">
        ...
        <nav className="flex items-center gap-2 sm:gap-6 text-sm font-body">
          {[
            ...
            >
              {item.label}
              {currentView === item.id && (
                <span className="absolute bottom-0 left-0 w-full h-0.5 bg-dd-primary rounded-full" />
              )}
            </button>
          ))}
        </nav>
